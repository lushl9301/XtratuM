...
#define _CPUID_XMM (1<<25)
#define _CPUID_XMM2 (1<<26)
...
struct x86DescReg {
    xm_u16_t limit;
    xmAddress_t linearBase;
} __PACKED;

#ifdef CONFIG_SMP
#define GET_CPU_ID()        __ArchGetLocalId()
#define SET_CPU_ID(x)       __ArchSetLocalId(x)
#define GET_CPU_HWID()      __ArchGetLocalHwId()
#define SET_CPU_HWID(x)     __ArchSetLocalHwId(x)
#else
#define GET_CPU_ID()        0
#define SET_CPU_ID(x)
...
/*
#define x86SystemPanic(...) do { \
        cpuCtxt_t ctxt; \
        GetCpuCtxt(&ctxt); \
        SystemPanic(&ctxt, __VA_ARGS__); \
    } while (0)
*/
#define DCACHE 0
#define ICACHE 0
...
#define x86SystemPanic(...) do { eprintf(__VA_ARGS__); HaltSystem(); } while(0)

extern void EarlyDelay(xm_u32_t cycles);
extern xm_u32_t __ArchGetLocalId(void);
extern xm_u32_t __ArchGetLocalHwId(void);
extern void __ArchSetLocalId(xm_u32_t id);
extern void __ArchSetLocalHwId(xm_u32_t hwId);
