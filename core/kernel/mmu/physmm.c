	...
	for (l = 0, r = xmcTab.noRegions - 1; l <= r;) {
		...
		if (pAddr < a) {
			...
			if (pAddr > b) {
				...
				if (!(xmcMemRegTab[c].flags & XMC_REG_FLAG_PGTAB))
					return 0;
	...
	for (l = 0, r = cfg->noPhysicalMemoryAreas - 1; l <= r;) {
		...
		if (pAddr < a) {
			...
			if (pAddr > b) {
				...
				return &physPageTab[memArea->memoryRegionOffset][(pAddr - memRegion->startAddr)
						>> PAGE_SHIFT];
			}
		}
	}
	...
	for (e = 0; e < p->cfg->noPhysicalMemoryAreas; e++) {
		...
		if (memRegion->flags & XMC_REG_FLAG_PGTAB)
			...
			for (addr = memArea->startAddr; addr < memArea->startAddr + memArea->size; addr += PAGE_SIZE) {
				page = &physPageTab[memArea->memoryRegionOffset][(addr - memRegion->startAddr)
						>> PAGE_SHIFT];
				SpinLock(&page->lock);
				page->type = PPAG_STD;
				page->counter = 0;
