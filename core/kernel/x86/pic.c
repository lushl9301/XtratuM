...
static void PicEnableIrq(xm_u32_t irq) {
	...
	cMasterMask&=~(1<<irq);
	OutB(cMasterMask, master_ocw);
    } else {
	cMasterMask&=~(1<<CTLR2_IRQ);
	cSlaveMask&=~(1<<(irq-LAST_IRQ_IN_MASTER-1));
	OutB(cMasterMask, master_ocw);
	OutB(cSlaveMask, slave_ocw);
    }
...
void HwIrqSetMask(xm_u32_t mask) {
    cMasterMask = mask&0xff;
    cSlaveMask = (mask>>8)&0xff;
    if (cSlaveMask!=0xff)
        cMasterMask&=~(1<<CTLR2_IRQ);
    OutB(cMasterMask, master_ocw);
    OutB(cSlaveMask, slave_ocw);
